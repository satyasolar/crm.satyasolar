...
import { UserPlus, X, Shield, User, Mail, Phone, Calendar, Tag } from "lucide-react";
      ...
      <div className="modal-card" style={{ maxWidth: "500px", maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
          <form onSubmit={handleAddSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div>
              <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: 700, color: "#1e293b", marginBottom: "8px", letterSpacing: "-0.01em" }}>
                <Tag style={{ width: "13px", height: "13px", color: "#2563EB" }} /> Employee Initials <span style={{ color: "#6b7280", fontSize: "11px", fontWeight: 500, marginLeft: "4px" }}>(auto-generated, editable)</span>
              </label>
              <div style={{ position: "relative" }}>
                <Tag style={{ position: "absolute", left: "13px", top: "50%", transform: "translateY(-50%)", width: "15px", height: "15px", color: focusedField === "initials" ? "#2563EB" : "#94a3b8", pointerEvents: "none", transition: "color 0.2s" }} />
                <input
                  type="text"
                  maxLength={4}
                  style={focusedField === "initials" ? { ...fieldFocusStyle, textTransform: "uppercase" } : { ...fieldStyle, textTransform: "uppercase" }}
                  placeholder="e.g. RS (auto from name)"
                  value={formData.employee_initials || ""}
                  onFocus={() => setFocusedField("initials")}
                  onBlur={() => setFocusedField("")}
                  onChange={(e) => setFormData({ ...formData, employee_initials: e.target.value.toUpperCase().replace(/[^A-Z]/g, "") })}
                />
              </div>
              <p style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px", marginLeft: "2px" }}>2–3 letters shown on case timeline. Leave blank to auto-generate from name.</p>
            </div>
