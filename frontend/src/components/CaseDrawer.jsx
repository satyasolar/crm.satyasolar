...
const stageToDeptLabel = {
  ...
  field: "Field Operations Department",
  qa: "QA Department",
...
const CaseDrawer = ({ caseData, onClose, onRefresh }) => {
  ...
  const canUpdate = role === "admin" || ownerRole === role;
  ...
  // ── Mandatory 5 documents that must be uploaded by Sales before forwarding ──
  ...
  const hasMissingMandatoryDocs =
    role === "sales" && isRegStage && missingMandatoryDocs.length > 0;

  const unverifiedDocs = Object.keys(docs).filter(
    (d) => (docStatuses[d] || "Yellow") !== "Green",
  );
  const hasDocError = (isRegStage && unverifiedDocs.length > 0) || hasMissingMandatoryDocs;

  // Quotation must be verified before proceeding from registration stages
  const hasQuotationError = isRegStage && !normalized.quotationVerified;

  const isBankingStage = normalized.currentStage === "Finance Clearance";
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    if (hasQuotationError) {
      toast.error("Please verify the quotation amount before proceeding.");
      return;
    }
