    ...
    const load = async () => {
      try {
        if (isSimulating || !isAdmin) {
          ...
          const [empStats, empCases, empActivity, empQuotes] =
          ...
          if (es) {
            ...
            if (userRole === "sales") {
              ...
              setStats({
                ...
                inProgressCases: rawCases.filter(c => c.status !== "Completed" && c.status !== "Delayed" && c.currentStage !== "Completed").length,
                completedCases: rawCases.filter(c => c.status === "Completed" || c.currentStage === "Completed").length,
                delayedCases: rawCases.filter(c => c.status === "Delayed").length,
              });
            } else {
        ...
        if (userRole === "sales") {
          ...
          setStats({
            ...
            inProgressCases: rawQuotes.filter(
              (q) => q.status === "Processing" || q.status === "In Process",
            ).length,
            completedCases: rawQuotes.filter(
              (q) =>
                q.current_department === "Registration" ||
                q.status === "Registration Completed" ||
                q.status === "Sent to Registration",
            ).length,
            delayedCases: rawQuotes.filter((q) => q.status === "Rejected")
              .length,
          });
        } else {
          setStats(getData(0));
    ...
    registration: [
      {
        ...
        val: cases.filter((c) => c.currentStage === "Registration Done").length,
        color: "#6366f1",
        icon: Clock,
      },
      {
        ...
        val: cases.filter(
          (c) =>
            c.currentStage === "Registration Done" && c.status === "Delayed",
        ).length,
        color: "#f43f5e",
        icon: AlertTriangle,
      },
