...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (selfServiceActions.includes(action)) {
      ...
      if (action === "get_own_details") {
        const { data: profile, error } = await supabaseServiceRole
          .from("profiles")
          .select("id, name, role, phone, dob, designation, status, is_head, employee_id, employee_initials, city, state, country, address, gender, emergency_contact_name, emergency_contact_phone, bank_name, branch_name, account_number, ifsc_code, experience_years, company_name, work_location, last_ctc, aadhar_card, pan_card, created_at")
          .eq("id", user.id)
          .single();
    ...
    if (action === "create_user") {
      ...
      // Auto-generate initials from name (first letter of each word, max 3 chars)
      const autoInitials = body.employee_initials || name.split(" ").filter((w: string) => w.length > 0).map((w: string) => w[0].toUpperCase()).join("").slice(0, 3);
      ...
      const { error: profileError } = await supabase.from("profiles").upsert({
        ...
        employee_initials: autoInitials,
      });
  ...
  <div class="footer">
    ...
    if (action === "list_users") {
      ...
      let query = supabase
        .from("profiles")
        .select(
          "id, name, role, phone, dob, status, is_first_login, created_at, employee_id, employee_initials, is_head, designation, suspended_from, suspended_until",
        );
      ...
      const users = authUsers
        ...
        return {
          ...
          employeeInitials: p?.employee_initials || "",
        };
      });
