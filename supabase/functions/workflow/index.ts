...
const legacyStageMap: Record<string, string> = {
  ...
  "Project: Survey & Design": "Project Phase 1",
  "Survey Completed": "Project Phase 1",
  "Design & BOM Approved": "Project Phase 1",
  "Warehouse: Material Dispatch": "Warehouse",
  "Material Reserved": "Warehouse",
  "Project: Installation": "Project Phase 2",
  "Structure Installed": "Project Phase 2",
  "Full Installation Completed": "Project Phase 2",
  ...
  "Customer Service Update": "Customer Service / AMC",
...
<div class="footer">© ${new Date().getFullYear()} ${companyName}. This is an automated message.</div>
    ...
    if (action === "update_stage") {
      ...
      const currentNormalized = normalizeStage(currentStageRaw);
      const normalizedStage = workflowStages.find(
        (s) => s.toLowerCase() === currentNormalized.toLowerCase(),
      );
      if (!normalizedStage)
        return jsonResponse(
      ...
      if (
        normalizedStage === "Sales" ||
        normalizedStage === "Registration"
      ) {
      ...
      // RBAC check: sales can move case FROM "Sales" to "Registration"
      const isSalesHandoff =
        user.role === "sales" &&
        normalizedStage === "Sales" &&
        normalizeStage(body.newStage?.trim()) === "Registration";
      ...
      const isRegStage = normalizedStage === "Registration";
      let expectedNextStage: string;

      if (isRegStage) {
        const payType = (body.paymentType || caseObj.payment_type || "").toLowerCase();
        if (payType === "loan") {
          expectedNextStage = "Banking & Finance";
        } else {
          expectedNextStage = "Project Phase 1";
        }
      } else if (normalizedStage === "Banking & Finance") {
        expectedNextStage = "Project Phase 1";
      } else {
        expectedNextStage = workflowStages[currentIndex + 1];
      }

      const requestedStageNormalized = normalizeStage(body.newStage?.trim());
      if (
        expectedNextStage.toLowerCase() !== requestedStageNormalized.toLowerCase()
      ) {
        return jsonResponse(
          {
      ...
      const isProjectComplete = expectedNextStage === "Project Closed";
      ...
      // If project complete, add a second history entry marking as Completed
      if (isProjectComplete) {
        await supabase.from("case_history").insert({
          ...
          remarks:
            "Customer automatically marked Completed after Customer Service / AMC",
        });
      }

      // ── AUTO-INVENTORY DEDUCTION FOR "Warehouse" ────────────────────
      if (finalStage === "Warehouse") {
        try {
...
<div class="footer">© ${new Date().getFullYear()} ${companyName}. This is an automated message.</div>
    ...
    if (action === "delete") {
      ...
      const stageNormalized = normalizeStage(caseObj.current_stage);
      if (stageNormalized !== "Sales") {
        return jsonResponse(
          {
            message: "Case can only be deleted at the Sales stage.",
          },
          403,
        );
      }
